<?php 
  ...
  if(isset($_POST['save'])) {
    ...
    $query=mysqli_query($conn,"select * from class where className ='$className' and Id = '$Id'");
    $ret=mysqli_fetch_array($query);

    if($ret > 0) { 
      $statusMsg = "<div class='alert alert-danger' style='margin-right:700px;'>This Class Arm Already Exists!</div>";
    } else {
      $query=mysqli_query($conn,"insert into class(className) value('$className')");
  ...
  if (isset($_GET['Id']) && isset($_GET['action']) && $_GET['action'] == "edit") {
    ...
    if(isset($_POST['update'])){
      ...
      $query=mysqli_query($conn,"update class set className='$className' where Id ='$Id'");
